...
@Component({
    ...
    providers: [
        { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => PaymentCardComponent), multi: true },
        { provide: NG_VALIDATORS, useExisting: forwardRef(() => PaymentCardComponent), multi: true }
    ],
})
export class PaymentCardComponent implements ControlValueAccessor {
    ...
    validateCard(): boolean {
        ...
        return this.holder
            && this.number?.length >= 14 && this.number?.length <= 19
            && this.code?.length === 3
            && this.month <= 12
