...
export class PasswordStrength {
    constructor(
        readonly length: number | MinMax = [8, 20],
        readonly lower: number | MinMax = 1,
        readonly upper: number | MinMax = 1,
...
export function generatePassword(pwdStrength?: PasswordStrength): string {
    ...
    const generators: Partial<Record<"upper" | "special" | "digit", { range: [number, number]; chars: string[] }>> = Object.entries(pwdStrength)
        .filter((x) => x[0] in charsGroups)
        .map((x) => {
    ...
    let chars = Object.entries(generators)
        .map((c) => c[1].chars.join(""))
        .join("");
    ...
    return chars
        .split("")
        .sort((c1, c2) => Math.random() - Math.random())
        .join("");
}
