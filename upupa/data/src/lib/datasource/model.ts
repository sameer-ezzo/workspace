...
export abstract class TableDataSource<T = any> implements ITableDataSource<T> {
    ...
    set page(page: PageDescriptor) { this.page$.next(page); if (this._initialized) this.refresh(); }

    readonly sort$: BehaviorSubject<SortDescriptor> = new BehaviorSubject<SortDescriptor>(null);
    get sort(): SortDescriptor { return this.sort$.value; }
    set sort(sort: SortDescriptor) { this.sort$.next(sort); if (this._initialized) this.refresh(); }

    readonly filter$: BehaviorSubject<FilterDescriptor> = new BehaviorSubject<FilterDescriptor>({});
    get filter(): FilterDescriptor { return this.filter$.value; }
    ...
    get terms(): Term<T>[] { return this.terms$.value; }
