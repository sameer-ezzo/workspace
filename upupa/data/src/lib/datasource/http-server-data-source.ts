...
export class HttpServerDataSource<T = any> extends TableDataSource<T> {
    ...
    constructor(private readonly http: HttpClient,
        ...
        private readonly resToData: (res: any) => T[] = res => res.data
    ) {
        super();
    }
    ...
    refresh(): Observable<T[]> {
        ...
        if (term) {
            ...
            if (termsFields.length) {
                ...
                if (v)
                    query[k.field] = v;
                if (termsFields.length)
                    query[k.field] += '|' + termsFields.map(f => `${String(f.field)}=${this._evalTerm(f, term)}`).join("|");
            }
        }
        ...
        if (sort?.active)
            query.sort_by = `${sort.active},${sort.direction}`;
    ...
    private doRequest(params: any) {
        const qps = Object.entries({ ...this.filter, ...params }).map(([k, v]) => `${k}=${v}`).join('&');
    ...
    override getItems(value: (string | number | symbol)[], key: string | number | symbol): Observable<T[]> {
        return value?.length > 0 ?
            this.doRequest({ [key]: `{in}${value.join(',')}` })
            : of([])
    }
}
