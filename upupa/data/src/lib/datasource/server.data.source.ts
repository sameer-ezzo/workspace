...
export class ServerDataSource<T = any> extends TableDataSource<T> {
  ...
  readonly data$ = this.src$.pipe(
    debounceTime(200),
    switchMap((src) => src.pipe(map((x) => x))),
  ...
  refresh(): Observable<T[]> {
    ...
    const query: any = {};
    const term: any =
      filter && filter.terms?.length ? filter.terms.join(' ') : '';
    if (term) {
    ...
    if (this.selectedColumns?.length > 0)
      query.select = this.selectedColumns.join(',');
  ...
  override getItems(
    ...
    return value?.length > 0
      ? this.getData(this.path, {}, { [key]: `{in}${value.join(',')}` })
      : of([]);
  }
}
