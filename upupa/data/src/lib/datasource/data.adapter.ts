...
export class DataAdapter<T = any> extends Normalizer<T, NormalizedItem<T>> {
  ...
  getItems(keys: (keyof T)[]): Promise<NormalizedItem<T>[]> {
    ...
    if (itemsInAdapter.length === KEYS.length)
      return Promise.resolve(normalized.filter((n) => KEYS.includes(n.key)));

    const source = this.dataSource
      .getItems(itemsNotInAdapter, this.keyProperty)
      .pipe(
        map((items) =>
          items.filter(
            (x) =>
  ...
  _normalizeItem(item): NormalizedItem<T> {
    ...
    let valueProperty = Array.isArray(this.valueProperty)
      ? [this.keyProperty, ...this.valueProperty]
      : [this.keyProperty, this.valueProperty];
    ...
    if (valueProps.length > 1) value = this.extract(item, valueProperty, item);
      ...
      value = valueProps[0] ? item[valueProps[0]] : item; //to handle keyProperty = null (take the item itself as a key)
  ...
  private __extract(item: any, property: any) {
    if (property.indexOf('.') > -1)
      return JsonPointer.get(item, property.replaceAll('.', '/'));
