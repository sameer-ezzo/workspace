...
import { addEntities, updateEntity, removeEntities, setAllEntities, setEntities, setEntity, withEntities, removeEntity } from "@ngrx/signals/entities";
...
export class DataAdapter<T = any> extends DataAdapterStore<any>() {
    ...
    normalize(item: T): NormalizedItem<T> {
        ...
        const image = this.extract(item, this.imageProperty, undefined);
        return { key, display, value, image, item:signal(item) };
    }
