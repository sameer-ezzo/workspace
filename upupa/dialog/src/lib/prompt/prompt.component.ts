...
export class PromptComponent
  implements UpupaDialogPortal<PromptComponent>, OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (data.type !== null) {
      if (this.type === 'number')
        this.valueFormControl = new FormControl<number>(+(data.value || '0'), [
          ...validators,
        ]);
      else if (this.type === 'email')
        this.valueFormControl = new FormControl<string>(data.value || '', [
          ...validators,
          Validators.email,
        ]);
      else
      ...
      .subscribe((v) => {
        this.dialogActions.set(
          this.dialogActions().map((a) => {
            ...
            if (a.type === 'submit')
              action.disabled = this.valueFormControl.invalid;
            return action;
          }),
        );
      });
