        ...
        if (this.dialogData.inputs) {
            ...
            if (inputsKeys.length > 0) {
                ...
                for (const inputName of inputsKeys) {
                    ...
                    } else console.warn(`The component ${meta.type.name} is missing an input named: ${inputName}`);
                }
        ...
        if (this.dialogData.outputs) {
            ...
            if (outputsKeys.length > 0) {
                for (const outputName of outputsKeys) {
                    const output = outputs.find((i) => i.propName === outputName || i.templateName === outputName);
                    if (output)
                        this.component[outputName].pipe(takeUntil(this.dialogRef.afterClosed())).subscribe((e) => {
                    ...
                    else console.warn(`The component ${meta.type.name} is missing an output named: ${outputName}`);
                }
            }
        }
    }
