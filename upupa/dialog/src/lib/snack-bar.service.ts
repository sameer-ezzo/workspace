...
export class SnackBarService {
    constructor(private snack: MatSnackBar, private t: TranslateService) { }
    ...
    openFailed(message: string = 'failed', error?: any, config?: SnackbarConfig) {
        const m = this.t.translate(message);
        if (config && config.callback) {
            return this.openAction(m, this.t.translate(config.callbackName || 'snkbar-failed-msg'),
    ...
    openConfirm(message: string, onConfirmed: () => void) {
        return this.openAction(this.t.translate(message), this.t.translate('yes'), () => { onConfirmed() })
    }
