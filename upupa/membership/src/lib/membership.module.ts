...
const optionsProviders = <T extends BaseMembershipFormOptions>(
    form: 'LOG_IN' | 'SIGNUP' | 'FORGOT_PASSWORD' | 'VERIFY',
    options: T
): Provider[] => {
    const providers = {
        LOG_IN: [
...
const membershipOptionsProviders = (options: MembershipOptions): Provider[] => {
    ...
    if (options.login !== null)
        providers.push(
            ...optionsProviders<MembershipLoginOptions>('LOG_IN', options.login)
        );
    if (options.signup !== null)
        providers.push(
            ...optionsProviders<MembershipSignupOptions>(
                'SIGNUP',
                options.signup
            )
        );
    if (options.verify !== null)
        providers.push(
            ...optionsProviders<MembershipVerifyOptions>(
                'VERIFY',
                options.verify
            )
        );
    if (options.forgotPassword !== null)
        providers.push(
            ...optionsProviders<MembershipForgotPasswordOptions>(
                'FORGOT_PASSWORD',
                options.forgotPassword
            )
        );
    if (options.idPs !== null)
        providers.push({ provide: IdPs_OPTIONS, useValue: options.idPs });
...
export class MembershipModule {
    public static forRoot(
    ...
    ): ModuleWithProviders<MembershipModule> {
        const mergeOptions = (
            key: keyof MembershipOptions,
            options,
            defaults
        ) => {
        ...
        if (options) {
            membershipOptions.login =
                options.login === null
                    ? null
                    : mergeOptions('login', options, _defaultMembershipOptions);
            membershipOptions.signup =
                options.signup === null
                    ...
                    : mergeOptions(
                          'signup',
                          options,
                          _defaultMembershipOptions
                      );
            membershipOptions.verify =
                options.verify === null
                    ? null
                    : mergeOptions(
                          'verify',
            ...
            membershipOptions.forgotPassword =
                options.forgotPassword === null
                    ? null
                    : mergeOptions(
                          'forgotPassword',
                          options,
            ...
            membershipOptions.idPs =
                options.idPs === null
                    ? null
                    : { ..._defaultMembershipOptions.idPs, ...options.idPs };
        }

        const _membershipOptionsProviders =
            membershipOptionsProviders(membershipOptions);
