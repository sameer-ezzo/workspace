...
export class DataListFilterComponent implements AfterViewInit, OnDestroy {
  ...
  private toBase64(obj): string {
    return btoa(JSON.stringify(obj));
  }
  private fromBase64(str: string): any {
  ...
  get page() {
    return (this.dataListResolverResult.adapter.page?.pageIndex ?? 0) + 1;
  }
  get per_page() {
  ...
  private async refreshData() {
    if (
      this.dataListResolverResult.adapter.dataSource instanceof ServerDataSource
    ) {
      const dpath = (
