...
import { DynamicFormComponent, FieldItem, Fieldset, FormScheme } from '@upupa/dynamic-form';
...
export class DataFilterFormComponent extends InputBaseComponent<Record<string, any>> {
    ...
    constructor() {
        ...
        this.valueChange$.pipe(
            debounceTime(Math.max(0, this.filterChangeDebounceTime ?? 0))
        ).subscribe(x => this._formValueChangeHandler(x))
    }
    ...
    function fromFieldItem(field: FieldItem, value: any, path: string): { [path: string]: string } {
        ...
        if (_adapter || adapter)
            if (adapter.keyProperty) {
                ...
                v = (value ?? []).map(x => (x[adapter.keyProperty] || x)).filter(c => c).join('|')
            }
            else {
    ...
    function fromFieldSet(items: FormScheme, value: any, path: string): FilterDescriptor {
        ...
        for (const [name, field] of Object.entries(items)) {
            ...
            if (field.type === 'fieldset') {
                ...
                fd = { ...fd, ...fromFieldItem(field, v, toPath(path, name)) }
        }
        return fd
    }
