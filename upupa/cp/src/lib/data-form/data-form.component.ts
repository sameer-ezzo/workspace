...
export class DataFormComponent<T = any>
    implements UpupaDialogPortal<DataFormComponent<T>>
{
    ...
    public set form(dForm: DynamicFormComponent) {
        ...
        this._formChangesSub = dForm
            ...
            .valueChanges.pipe(
                ...
                map((v) => (dForm.form().valid ? 'VALID' : 'INVALID'))
            )
            .subscribe((status) => {
                this.dialogActions.set(
                    this._actions.map((a) =>
                        a.type === 'submit'
                            ? { ...a, disabled: status !== 'VALID' }
                            : a
                    )
                );
            });
    ...
    init(changes: Record<string, unknown>) {
        ...
        if (changes['actions'])
            this.actions = changes['actions'] as ActionDescriptor[];
        if (changes['conditions'])
            this.conditions = changes['conditions'] as Condition<
                DynamicFormEvents.AnyEvent,
                DynamicFormCommands.AnyCommands
            >[];
        if (changes['valueToRecord'])
            this.valueToRecord = changes['valueToRecord'] as (
                form: DynamicFormComponent,
                value: T
            ) => Promise<any>;
        if (changes['onSubmit'])
            this.onSubmit = changes['onSubmit'] as (
                path: string,
                record: any
            ) => Promise<FormSubmitResult>;
        if (changes['defaultSubmitOptions'])
            this.defaultSubmitOptions = changes[
        ...
        if (
            !this.fields ||
            Object.getOwnPropertyNames(this.fields).length === 0
        )
            throw new Error('DataFormComponent requires a FormScheme');
        if ((this.actions || []).length === 0)
            this.actions = defaultFormActions;
    }
