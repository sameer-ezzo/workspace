...
import {
  getListInfoOf,
  resolvePathScaffolders,
} from '../decorators/scheme.router.decorator';
import {
  ListViewModelOptions,
  LookUpDescriptor,
  QueryType,
...
export class BaseListViewScaffolder
  implements IScaffolder<ListScaffoldingModel>
{
  ...
  async scaffold(
    path: string,
    ...params: any[]
  ): Promise<ListScaffoldingModel> {
    const { path: _path } = resolvePath(path);
    const collection = _path
      .split('/')
      .filter((s) => s)
      .pop();
    const listInfo = getListInfoOf(collection) as any;

    const evaluatedQuerySelector =
      await this.authorizeService.getEvaluatedQuerySelector(
        `api/${collection}`,
        'Read',
    ...
    if (listInfo.filterForm) {
      ...
      if (f) {
        ...
        ffVm = {
          ...
          ...(filterForm as DataListFilterForm),
          fields,
        };
      }
    }
...
function resolveQueryFn(
  ...
  const lookups = Array.from(Object.entries(listInfo.columns ?? {}))
    .filter(([k, v]) => v['lookup'])
    .map(([k, v]) => v['lookup'] as LookUpDescriptor)
    .map(
      (l: LookUpDescriptor) =>
        `${l.from}:${l.foreignField}:${l.localField}:${l.as || l.localField}:${
          l.single === true ? 'unwind' : ''
        }`
    )
    .filter((l) => l.length)
    .join(';')
  ...
  const v =
    lookups.length > 0
      ? { ...qs, lookup: lookups, ...base }
      : { ...qs, ...base };
  const queryFn = () => v;
  return { query: queryFn };
}
function resolveQueryParamsFn(
  listInfo: Partial<ListViewModelOptions>,
  base: any
