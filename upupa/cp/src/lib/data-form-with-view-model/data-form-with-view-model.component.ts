import { Component, ViewChild, inject, DestroyRef, signal, computed, input, Injector, effect } from "@angular/core";
import { DynamicFormComponent, DynamicFormModule, resolveDynamicFormOptionsFor } from "@upupa/dynamic-form";
...
export class DataFormWithViewModelComponent<T = any> implements UpupaDialogPortal<DataFormWithViewModelComponent<T>> {
    ...
    async submit() {
        ...
        if (!vm[submitAction.handlerName]) throw new Error(`Handler ${submitAction.handlerName} not found in ViewModel`);
