...
import { BehaviorSubject, catchError, combineLatest, debounceTime, firstValueFrom, from, map, Observable, ReplaySubject, switchMap, tap } from 'rxjs'
...
export class UserAvatarService {
    ...
    private readonly _avatarModeChanged = new BehaviorSubject<AvatarMode>('avatar')
    private _avatarMode: AvatarMode;
    public get avatarMode(): AvatarMode { return this._avatarMode; }
    ...
    async getAvatar(mode: AvatarMode, principle: Principle): Promise<any> {
        if (!principle) return ''
        switch (mode) {
            case 'avatar': return await firstValueFrom(
                this.dataService.get<FileInfo[]>(`/user/${principle.sub}/picture`)
                .pipe(
                    map(userAvatars => `${base}/${userAvatars[0].path}?view=1&w=100&h=100&access_token=${this.auth.get_token()}`),
                    catchError(e => from(this.getAvatar('initials', principle)))))
            case 'initials': return this.generateInitialsAvatar(principle.name ?? principle.email)
            default: return ''
        }
    }
    ...
    generateInitialsAvatar(userName: string, initialsCount = 2) {
