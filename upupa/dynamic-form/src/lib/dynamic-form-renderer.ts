...
export class DynamicFormRenderer<T = any> {
    ...
    processFieldItem(
        ...
        if (!control) {
            const componentMap = this.formService.getControl(
                field.input,
                this.theme
            );
    ...
    private getValidators(field: Field) {
        ...
        if (isRequired)
            validations.push({ name: 'required', message: 'required' });
    ...
    private setupValidationTask(field: FieldItem, control: AbstractControl) {
        ...
        control.valueChanges.subscribe(() => {
            ...
            task.disabled = !(
                errorCodes.length === 1 &&
                control.errors[errorCodes[0]].validationTask
            );
        });
