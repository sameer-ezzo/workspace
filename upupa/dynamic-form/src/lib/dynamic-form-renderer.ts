...
import { AbstractControl, UntypedFormBuilder, ValidatorFn, UntypedFormGroup, UntypedFormControl, FormGroup } from '@angular/forms';
...
import { Field, Fieldset, FieldItem, ValidationTask, Validator, FormScheme } from './types';
...
//     private get theme() {
//         return this.parent?.theme() ?? this.defaultTheme;
//     }
...
//                 if (formGroup && control === formGroup.controls[field.name]) {
//                     //double checking on control because removing is by name
...
//         if (isRequired) validations.push({ name: 'required', message: 'required' });
...
//             if (errors === null && task.state != 'check') {
//                 //if normal validators are happy and task still didn't sccuceed then show validation-task error
//                 errors = {};
//                 errors[task.name || 'validation-task'] = {
//                     msg: task.error,
//                     validationTask: true,
//                 };
//             }
...
//             task.token = null;
//             if (task.confirm) task.state = task.confirm.state = 'vpn_key';
...
//         if (task.disabled || task.state === 'check') return;
...
//             if (result === null || result === true) task.state = 'check';
//             else if (result['code']) {
//                 task.state = 'vpn_key';
...
//             this.dialog
//                 .openDialog(TaskValidationComponent, {
//                     data: { field, task, control },
//                 })
//                 .afterClosed()
//         );
//     }
...
//             if (f.type === 'fieldset') this._writeOnFieldset(f, group as FormGroup, v);
...
export type BuilderField = { field: Field; control: UntypedFormControl | UntypedFormGroup; form: UntypedFormGroup; template?: DynamicComponent | undefined };
export type BuilderResult = {
    formGroup: UntypedFormGroup;
    fields: BuilderField[];
};
export class DynFormBuilder {
    constructor(private readonly fb: UntypedFormBuilder, private readonly formService: DynamicFormService, private readonly bus: EventBus) {}

    private _form: UntypedFormGroup = undefined;
    get form() {
        return this._form;
    }
    readonly controls: Map<Field, Omit<BuilderField, 'field'>> = new Map();

    build(scheme: FormScheme, theme: string, value: any): FormScheme {
        this.controls.clear();
        this._form = this.fb.group({});
        this.processFields('', scheme, theme, value, this.form);

        return scheme;
    }

    getBuilderField(field: Field): Omit<BuilderField, 'field'> {
        return this.controls.get(field);
    }
    private processFields(path: string, scheme: FormScheme, theme: string, value: any, parentForm: UntypedFormGroup): void {
        const fieldsArr = Object.values(scheme);
        for (const f of fieldsArr) {
            if (f.type === 'fieldset') this.processFieldset(path, f, theme, value, parentForm);
            else this.processFieldItem(path, f, theme, value, parentForm);
        }
    }

    private processFieldset(path: string, fieldset: Fieldset, theme: string, value: any, parentForm: UntypedFormGroup): void {
        if (!fieldset['path']) fieldset['path'] = `${path}/${fieldset.name}`;
        let nestedForm = parentForm.controls[fieldset.name] as UntypedFormGroup;
        if (!nestedForm) {
            nestedForm = this.fb.group({});
            this.form.addControl(fieldset.name, nestedForm);
        }
        const template = {
            component: undefined,
            inputs: fieldset.ui.inputs,
            outputs: fieldset.ui.outputs,
            class: fieldset.ui.class,
        };
        this.controls.set(fieldset, {
            control: nestedForm,
            form: parentForm,
            template,
        });

        const v = value ? JsonPointer.get(value, fieldset.path) : undefined;
        this.processFields(fieldset.path, fieldset.items, theme, v, nestedForm);
    }

    private processFieldItem(path: string, field: FieldItem, theme: string, value: any, parentForm: UntypedFormGroup): void {
        if (!field['path']) field['path'] = `${path}/${field.name}`;
        const componentMap = this.formService.getControl(field.input, theme);
        if (componentMap.field) _mergeFields(field, componentMap.field);

        const v = value ? JsonPointer.get(value, field.path) : undefined;
        const control = this.fb.control(v);
        ...
        control.markAsPristine();
        control.markAsUntouched();
        if (parentForm.controls[field.name]) parentForm.removeControl(field.name);
        parentForm.addControl(field.name, control);
    }

    getValidator(validator: Validator, field: Field): ValidatorFn {
        const validatorFactory = this.formService.getValidatorFactory(validator.name);
        if (validatorFactory) return validatorFactory(validator);
        else throw `Field ${field.path} has an invalid validator: ${validator.name}`;
    }
    private getValidators(field: Field) {
        const validations = field.validations || [];
        const isRequired = field.ui?.inputs?.['required'];
