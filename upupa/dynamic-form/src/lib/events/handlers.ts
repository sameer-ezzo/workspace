...
function changeState(form, cmd) {
    ...
    fs.forEach(f => {
        const control = controls.find(c => c[0].name === f)?.[1] as AbstractControl;
...
export function DynamicFormHandler<TCommand extends DynamicFormMessage>(targetForm: DynamicFormComponent, command: string, callback: (form: DynamicFormComponent, cmd: EventRecord<TCommand>) => void) {
    return targetForm.bus.on<TCommand>(_pattern(command))
        .subscribe(cmd => {
            ...
            if (cmd.match) {
                ...
                if (targetFieldName && !payload.fields?.length) payload.fields = [targetFieldName];
                if (targetFormName && !payload.targetFormName) payload.targetFormName = targetFormName;
            }

            //only apply commands to targeted form or otherwise apply it on the source
            if (payload.targetFormName && payload.targetFormName !== targetForm.name) return;
...
export function InputVisibilityHandler(targetForm: DynamicFormComponent) {
    return DynamicFormHandler<DynamicFormCommands.ChangeVisibility>(targetForm, DynamicFormCommands.changeVisibility, (form, cmd) => {
        for (const field of cmd.payload.fields) {
            const path = _path(field);
            JsonPointer.set(form.fields, `${path}/ui/hidden`, !cmd.payload.visibility);
        }
        })
}
...
export function ChangeFormSchemeHandler(targetForm: DynamicFormComponent) {
    ...
    return DynamicFormHandler<DynamicFormCommands.ChangeFormScheme>(targetForm, DynamicFormCommands.changeFormScheme, (form, cmd) => {
...
export function ChangeInputsHandler(targetForm: DynamicFormComponent) {
    return DynamicFormHandler<DynamicFormCommands.ChangeInputs>(targetForm, DynamicFormCommands.changeInputs, (form, cmd) => {
        const inputs = cmd.payload.inputs;
        const path = _path(cmd.payload.fields);
        const currentInputs = JsonPointer.get(form.fields, `${path}/ui/inputs`);
        JsonPointer.set(form.fields, `${path}/ui/inputs`, { ...currentInputs, ...inputs })
    })
}

export function ChangeStateHandler(targetForm: DynamicFormComponent) {
    return DynamicFormHandler<DynamicFormCommands.ChangeState>(targetForm, DynamicFormCommands.changeState, (form, cmd) => changeState(form, cmd))
}

export function ChangeValueHandler(targetForm: DynamicFormComponent) {
    return DynamicFormHandler<DynamicFormCommands.ChangeValue>(targetForm, DynamicFormCommands.changeValue, (form, cmd) => {
        const path = _path(cmd.payload.fields);
