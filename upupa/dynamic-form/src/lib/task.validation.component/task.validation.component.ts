...
export class TaskValidationComponent {
  ...
  field: FieldItem;
  constructor(public dialogRef: MatDialogRef<TaskValidationComponent>, @Inject(MAT_DIALOG_DATA) data: any) {
    this.task = data.task;
  ...
  async runConfirmationTask() {
    ...
    let result;
    try {
      result = await this.task.confirm.task(this.field.name, { value: this.control.value, code: this.code });
    ...
    catch (error) { this.task.confirm.state = 'error'; }

    this.task.disabled = false;
    this.control.updateValueAndValidity({ emitEvent: false });

    if (!this.task.confirm)
  ...
  async runValidationTask() {
    ...
    try {
      const result = await this.task.task(this.field.name, this.control.value);
    ...
    catch (error) { this.task.state = 'error'; }
