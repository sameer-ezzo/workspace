...
export class FileSelectComponent extends InputBaseComponent<FileInfo[]> implements OnDestroy {
    ...
    constructor(public readonly uploadClient: UploadClient,
        ...
        this.value1$.pipe(
            ...
            .subscribe(v => {
                this.viewModel = (v ?? []).map((f, id) => ({ id, file: f, error: null } as SelectInputFileVm))
            })

        this.clipboard.paste$.pipe(takeUntil(this.destroyed$))
    ...
    async uploadFileList(f: FileList) {
        const filesPs = Array.from(f).slice().map(async (file, id) => {
        ...
        const files = (await Promise.allSettled(filesPs))
            .map(f => {
    ...
    viewerEventsHandler(e: FileEvent) {
        if (e.name === 'removed') {
            e.files.forEach(f => this.viewModel.splice(this.viewModel.findIndex(c => c.id === f.id), 1))
            this.viewModel = this.viewModel.slice()
        }
