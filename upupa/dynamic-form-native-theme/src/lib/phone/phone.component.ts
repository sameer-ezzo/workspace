...
import { NG_VALUE_ACCESSOR, Validator, AbstractControl, ValidationErrors, NG_VALIDATORS } from '@angular/forms'
...
export type PhoneNumber = { raw: string, number: string, code: string, countryCode: number, isValid: boolean }
...
export class PhoneInputComponent extends InputBaseComponent implements Validator {

    inlineError = true

    @ViewChild('input') numberInput: ElementRef
   ...
   override validate(control: AbstractControl): ValidationErrors {
        return this.control.value.startsWith("+") ? { 'invalid-no': true } : null
    }
    ...
    onNumberInputChange(_value: string) {
        ...
        this.number = this.numberInput.nativeElement.value.trim()
