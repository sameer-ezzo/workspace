...
function matchPermissionSelectors(permission: Permission<boolean | AuthorizeFun>, ctx: AuthorizeContext): PermissionSelectorsEvaluationContext['match'] {
    ...
    if (match.result && query && Object.entries(query).some(([k, v]) => JsonPointer.get(msg.query, k) !== v))
        match.result = false


    if (match.result && payload && Object.entries(payload).some(([k, v]) => JsonPointer.get(msg.payload, k) !== v))
        match.result = false
...
export const AUTHORIZATION_TEMPLATES: Record<string, Pick<FunctionalPermission, 'authorize'>> = {
    ...
    role: {
        authorize: (ctx: AuthorizeContext, permission: StringValuePermission) =>
            ctx.msg?.principle?.roles?.some(r => r === permission.value)
    },
