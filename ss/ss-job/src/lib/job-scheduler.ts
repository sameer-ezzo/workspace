...
function getJobName(job: Job): string {
    return (job.opts as any).name ?? job.name;
}
...
export class JobScheduler {
    ...
    constructor(
        @Inject(JOB_SCHEDULAR_CONFIG) public readonly schedulerConfig: JobSchedulerConfig,
    ...
    private async _initCleanUpQueue() {
        if (!this._cleanUpQueue) {
            ...
            this._cleanUpQueue = this.createQueue(cleanUpQueueName, async () => {
    ...
    async run<T = any>(jobName: string, payload: any, jobOptions?: Exclude<JobOptions, { name: string }> & { queueName?: string }): Promise<Job<T>> {
        ...
        if (Array.isArray(processors) && processors.length > 0) {
            ...
            const job = (await queue.add({ payload: payload }, {
                timeout: 10000,
                attempts: processors.length,
                backoff: 200,
                ...jobOptions,
                name: jobName,
            } as JobOptions)) as Job<T>;
    ...
    async resolveProcessors(name: string) {
        ...
        try {
            const reolvedFromModule = await firstValueFrom(from(this.moduleRef.resolve<string, JobProcessor[]>(name)).pipe(timeout(500)));
    ...
    onQueueActive(job: Job): void {
        logger.info(`Processing job ${getJobName(job)} # ${job.id}`);
    }
    onQueueCompleted(job: Job, result: any): void {
        logger.info(`Job completed ${getJobName(job)} # ${job.id} with result: "${result}"`);
    }
    onQueueFailed(job: Job, err: any): void {
        logger.error(`Job ${getJobName(job)} # ${job.id} has failed with error: "${err}"`);
    }
    ...
    async process(job: Job): Promise<any> {
        ...
        let _processors = await Promise.all(
