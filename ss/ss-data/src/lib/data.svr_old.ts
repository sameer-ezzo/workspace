...
import type { DbModelDefinitionInfo } from './db-collection-info';
...
export class DataService {
  static async create(
    ...
    collections: DbModelDefinitionInfo,
  ...
  private async _migrate(ds: DataService, migrations: IDbMigration[]) {
    try {
      ...
      if (!model) return logger.warn(`Migration model not found!`);
      //  model = await ds.addModel('migration', migrationSchema);
      const migrationsInDb = await connection
  ...
  constructor(
    ...
    public readonly collections: DbModelDefinitionInfo,
  ...
  //     if (!collections['migration'])
  //       collections['migration'] = { schema: migrationSchema };
  //     if (!collections['tag']) collections['tag'] = { schema: tagSchema };
  //     if (!collections['change'])
  //       collections['change'] = { schema: changeSchema }; //todo: to be removed and registered in the constructor by the data change module or service if needed
  //     const keys = Object.keys(collections ?? {}).filter(
  //       (name) => name && collections[name]?.schema,
  //     );
  ...
  //           undefined,
  //           <string[]>this.collections[name].exclude,
  //         );
  //       } catch (error) {
  ...
  private async _updateModelsFromDb() {
    ...
    for (const collection in collections) {
      if (!this._models[collection]) {
        ...
        await this.addModel(collection, schema, '');
      }
    }
  }
  ...
  async addModel(
    ...
    const existingCollection = await this.connection
    ...
    if (existingCollection && existingCollection._id) {
      ...
      if (schemaIdType !== schema.paths._id.instance) {
        logger.error(
          `Schema: [${collectionName}] has different _id type. The existing id ${existingCollection._id} of type ${typeOfId} is different from the schema id type ${schema.paths._id.instance}!`,
        );
        return null;
      }
    }

    this._models[collectionName] = this.connection.model(
      collectionName,
      schema,
    );
    ...
    const migration = (this.migrations || []).filter(
      (m) => `${prefix}${m.collectionName}` === collectionName,
    );
  ...
  async getDynamicSchema(
    ...
    const extstingCollection = await this.connection
      .collection(cName)
      .findOne({});
  ...
  async find<T>(
    ...
    const model = await (this.options.autoCreateModel
    ...
    if (id) {
      ...
      try {
        return pointer ? JsonPointer.get(result, `/${pointer}`) : result;
      } catch (error) {
        return null;
      }
    } else {
  ...
  async func(path: string, f: 'count', ...q: { [key: string]: string }[]) {
    ...
    if (pathInfo.id)
      query['_id'] = this.convertToModelId(
        pathInfo.id,
        '_id',
        model,
        `func(${f}) ${path} ${q}`,
  ...
  public async post<T = any>(
    ...
    if (segments.id) {
      ...
      this.broker.emit<DataChangedEvent<T>>(`data-changed/${segments.path}`, {
        ...
        data: result.value,
        patches: [
          { op: 'add', path: <string>segments.pointer, value: newData },
        ],
        user,
      });
    ...
    if (typeof value !== 'object' || value === null) {
      ...
      for (const key in value) {
        if (value.hasOwnProperty(key)) {
          patches = patches.concat(
            this.generatePatches(value[key], `${currentPath}/${key}`),
          );
        }
      }
    }
  ...
  async put<T = any>(
    ...
    const id = <string>segments.shift();
    const patches = [
      { op: 'replace', path: '/' + segments.join('/'), value: value } as Patch,
    ];
    ...
    for (const by of inflateBy) {
      ...
      for (const inflated of inflatedItems) {
        ...
        for (const optionalField of optionalFields)
          if (!(optionalField in inflated)) inflated[optionalField] = undefined; //introduce the optional field to the obj to prevent future re-inflation attempt
      }
    }
