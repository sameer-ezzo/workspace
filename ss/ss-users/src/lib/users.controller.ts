...
export type SigninRequest = SigninRequestBase &
...
export class UsersController {
  ...
  public async userChanged(
    @Message() msg: IncomingMessage<DataChangedEvent<User>>,
  ) {
    ...
    if (new RegExp(`^/user/([a-z0-9]+)$`).test(path)) {
      if (patches.some((p) => p.path.includes('email')))
        await model.updateOne(
          { _id: data._id },
          { $set: { emailVerified: false, emailVerification: undefined } },
        );
      if (patches.some((p) => p.path.includes('phone')))
        await model.updateOne(
          { _id: data._id },
          { $set: { phoneVerified: false, phoneVerification: undefined } },
        );
    }
  }
  ...
  @EndPoint({
    http: { method: 'POST', path: 'forgotpassword' },
    operation: 'Forgot Password',
  })
  public async forgotPassword(
    @Message() msg: IncomingMessage<{ email: string }>,
  ) {
    ...
    await user.save();
    this.broker.emit(`${appName}/auth/forgotpassword`, {
      ...msg.payload,
      user,
      token: resetToken,
    });
  ...
  public async changePassword(
    ...
    if (!msg.payload)
      throw new HttpException('MISSING_INFO', HttpStatus.BAD_REQUEST);
  ...
  public async adminReset(
    ...
    const { access, rule, source } = this.authorizationService.authorize(
      msg,
      'update',
    );
    if (access === 'deny')
      throw new HttpException({ rule, source }, HttpStatus.FORBIDDEN);
  ...
  public async addUserToRoles(
    @Message() msg: IncomingMessage<{ userId: string; roles: string[] }>,
  ) {
    const { userId, roles } = msg.payload;
    if (!userId)
      throw new HttpException('MISSING_USER_ID', HttpStatus.BAD_REQUEST);
    const usersModel = await this.dataService.getModel('user');
    if (!usersModel)
      throw new HttpException('MISSING_USER_MODEL', HttpStatus.BAD_REQUEST);
  ...
  public async impersonate(@Message() msg: IncomingMessage<{ sub: string }>) {
    ...
    const user = (
      await this.auth.findUserById(msg.payload.sub)
    ).toObject() as User;
    ...
    return {
      ...
      refresh_token: await this.auth.issueRefreshToken(user, null, {
        imps: principle.sub,
      }),
    };
  }
  ...
  private async _doSignIn(msg: IncomingMessage<SigninRequest>): Promise<any> {
    ...
    switch (grantType) {
      case 'password': {
        ...
        if (!password && !msg.payload['id'])
          throw new HttpException('INVALID_PASSWORD', HttpStatus.BAD_REQUEST); //allow id signin without passing a password

        let user: User | null = null;
        if ('id' in msg.payload)
          user = await this.auth.signInUserByIdAndPassword(
            msg.payload.id,
            password,
            msg.payload.device,
          );
        else if ('username' in msg.payload)
          user = await this.auth.signInUserByUsernameAndPassword(
            msg.payload.username,
            password,
            msg.payload.device,
          );
        else if ('email' in msg.payload)
          user = await this.auth.signInUserByEmailAndPassword(
            msg.payload.email,
            password,
            msg.payload.device,
          );
        else if ('phone' in msg.payload)
          user = await this.auth.signInUserByPhoneAndPassword(
            msg.payload.phone,
            password,
            msg.payload.device,
          );
        ...
        if (!user)
          throw new HttpException(
            new AuthException(AuthExceptions.INVALID_ATTEMPT),
            HttpStatus.BAD_REQUEST,
          );
        ...
        return user.forceChangePwd === true
          ...
          : {
              access_token: await this.auth.issueAccessToken(user),
              refresh_token: await this.auth.issueRefreshToken(
                user,
                null,
                null,
      ...
      case 'refresh': {
        ...
        let user: User | null = null;
        if (token) {
          user = await this.auth.signInUserByRefreshToken(
            msg.payload.refresh_token,
          );
          if (token.d) {
            const currentDevice = user.devices?.[token.d];
            if (!currentDevice || currentDevice.active === false)
              throw new HttpException('INVALID_DEVICE', HttpStatus.BAD_REQUEST);
          }
          ...
          try {
            ...
            if (
              error.code == AuthExceptions.UserNotFound &&
              msg.ctx.authProvider?.shouldCreateUser(msg.principle)
            ) {
      ...
      default:
        throw new HttpException(
          AuthExceptions.InvalidGrantType,
          HttpStatus.BAD_REQUEST,
        );
    }
  }
  ...
  async googleAuthRedirect(@Req() req) {
    ...
    return {
      url: `${this.auth.options.externalAuth.google.client_url}/login?access_token=${access_token}&refresh_token=${refresh_token}`,
    };
  }
  ...
  async clientExternalAuth(@Body() req) {
    ...
    const GOOGLE_CLIENT_REGISTRATION_ENABLED =
      (
        process.env.GOOGLE_CLIENT_REGISTRATION_ENABLED || 'false'
      ).toLowerCase() === 'true';
  ...
  @Authorize({ by: 'anonymous', access: 'grant' })
  @EndPoint({
    http: { method: 'POST', path: 'fbacebook-auth' },
    operation: 'Auth with Facebook',
  })
  async fb_clientExternalAuth(@Body() req) {
    const fbuser = await this.http.get(
      'https://graph.facebook.com/v1.0' +
        '/me?fields=id,name,email' +
        '&access_token=' +
        req.access_token,
    );
    const inspectToken = await this.http.get(
    ...
    if (!userRecord) {
      const facebook_client_regeneration_enabled =
        (
          process.env.FACEBOOK_CLIENT_REGENERATION_ENABLED || 'false'
        ).toLowerCase() === 'true';
  ...
  async facebookLoginRedirect(@Req() req: Request) {
    ...
    return {
      url: `${this.auth.options.externalAuth.facebook.client_url}/login?access_token=${access_token}&refresh_token=${refresh_token}`,
    };
  }
  ...
  @Authorize({ by: 'anonymous', access: 'grant' })
  public async signup(
    @Message() msg: IncomingMessage<User & { password: string }>,
  ) {
    try {
      const _user = msg.payload;
  ...
  public async lock(
    @Message() msg: IncomingMessage<{ id: string; lock: string | boolean }>,
  ) {
  ...
  public async sendVerification(
    ...
    if (!name || !value)
      throw new HttpException('INVALID_ARGUMENTS', HttpStatus.BAD_REQUEST);
    ...
    if (!verification || now > verification.issuedAt + 1000 * 60 * 20) {
      const r = await this.auth.issueVerifyToken(
        user,
        name,
        value,
      ...
      verification.sendAttempts > 2 &&
      now - verification?.lastSend < 60 * 1000
    )
      throw new HttpException('ALREADY_SENT', HttpStatus.BAD_REQUEST);
  ...
  public async verify(
    ...
    try {
      ...
      if (type === 'code' && (!name || !value))
        throw new HttpException('INVALID_ARGS', HttpStatus.BAD_REQUEST);
...
); // TODO: verify user tokens signed in using google auth on client side
