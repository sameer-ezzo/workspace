...
export class UsersController {
    ...
    public async userChanged(@Message() msg: IncomingMessage<DataChangedEvent<User>>) {
        ...
        if (new RegExp(`^/user/([a-z0-9]+)$`).test(path)) {
            if (patches.some((p) => p.path.includes("email"))) await model.updateOne({ _id: data._id }, { $set: { emailVerified: false, emailVerification: undefined } });
            if (patches.some((p) => p.path.includes("phone"))) await model.updateOne({ _id: data._id }, { $set: { phoneVerified: false, phoneVerification: undefined } });
        }
    }
