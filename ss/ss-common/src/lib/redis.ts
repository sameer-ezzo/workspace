...
import { extractRedisHostsAndPorts } from "./read-default-redis-client-config";

export function parseRedisConfig(
    variableName: string
): { host: string; port: number }[] {
    const config = process.env[variableName];
    return config ? extractRedisHostsAndPorts(config) : [];
}
...
export class RedisClient {
    ...
    static async create(name: string, nodes: { host: string, port: number }[] = [], options: RedisOptions = defaultOptions): Promise<RedisClient> {
    ...
    constructor(public readonly name: string, public readonly options: RedisOptions = defaultOptions) {
    ...
    async connect(nodes: { host: string, port: number }[] = []) {
        ...
        this.redis.on("connect", () => { this._connect$.next(true) })
        this.redis.on("disconnect", () => { this._connect$.next(false) })
        ...
        if (nodes.length > 1)
            logger.warn(
                `REDIS:CONNECTED ${this.name} @ redis://${nodes
                    .map((n) => `${n.host}:${n.port}`)
                    .join(",")}`
            );
    ...
    private _onMessage() {
        ...
        this.redis.on("message", (channel, message) => {
            logger.verbose(
                `message received redis://${this.name} CHANNEL:${channel}, MSG:${message}`,
                { channel, message }
            );
    ...
    private _onPMessage() {
        ...
        this.redis.on("pmessage", (pattern, channel, message) => {
            logger.verbose(
                `pmessage received redis://${this.name} PATTERN:${pattern}, CHANNEL:${channel}, MSG:${message}`,
