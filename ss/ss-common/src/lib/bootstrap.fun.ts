...
import { SsFactory } from "./ss.factory"
...
const defaultAppBootRequirementsCheck = async () => {
    ...
    if (timezone !== 'UTC') 
        throw new Error(`Timezone must be set to UTC, current timezone is: ${timezone}, Please set the timezone to UTC`)
}

// BOOT APP/MICROSERVICE AND FORK WORKERS
export async function bootstrap(module: Type<unknown>, port = 3333, options?: Partial<AppOptions>, appBootRequirementsCheck: (...args: any[]) => Promise<void> = defaultAppBootRequirementsCheck) {
...
async function _bootstrap(applicationName: string, module: Type<unknown>, port = 3333, _options?: Partial<AppOptions>) {
    ...
    application = await SsFactory.create<NestExpressApplication>(module, httpAdapter, { logger, autoFlushLogs: true, bufferLogs: true })
    ...
    application.use(express.urlencoded({ extended: true, limit: sizeLimit, parameterLimit: 1000 }))
    if (options.middlewares)
        for (const mw of _options.middlewares)
            application.use(mw)
