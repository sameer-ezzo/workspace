...
export type AppOptions = {
  ...
  socketAdapter?:
    | 'default'
    | `REDIS_${string}`
    | ((app: INestApplicationContext) => IoAdapter);
...
const defaultAppBootRequirementsCheck = async () => {
  ...
  if (timezone !== 'UTC')
    throw new Error(
      `Timezone must be set to UTC, current timezone is: ${timezone}, Please set the timezone to UTC`,
    );
};
...
export async function bootstrap(
  ...
  appBootRequirementsCheck: (
    ...args: any[]
  ) => Promise<void> = defaultAppBootRequirementsCheck,
) {
  if (appBootRequirementsCheck) await appBootRequirementsCheck();
  const applicationName =
    options?.applicationName ?? process.env['APP_NAME'] ?? module.name;
...
async function _bootstrap(
  ...
  application = await new NestFactoryStatic().create<NestExpressApplication>(
    module,
    httpAdapter,
    { logger, autoFlushLogs: true, bufferLogs: true },
  );
  // application.useLogger(logger)

  const staticAssets = options.staticAssets ?? { assets: 'assets' };
  for (const p in staticAssets) {
    const assets_dir = staticAssets[p].startsWith('/')
      ? staticAssets[p]
      : join(__dirname, staticAssets[p]);
  ...
  application.use(
    express.json({ reviver: dateReviver, strict: false, limit: sizeLimit }),
  ); // To parse the incoming requests with JSON payloads (reviver automatically convert date string to date obj)
  ...
  if (options.middlewares)
    for (const mw of options.middlewares) application.use(mw);

  //START SOCKET ADAPTER
  if (options.websocketServer) {
    if (
      typeof options.socketAdapter === 'string' &&
      options.socketAdapter.startsWith('REDIS_')
    ) {
      const redisClient = (await application.get(
        options.socketAdapter,
      )) as RedisClient;
      application.useWebSocketAdapter(
        new RedisIoAdapter(application, redisClient),
      );
    } else if (typeof options.socketAdapter === 'function')
      application.useWebSocketAdapter(options.socketAdapter(application));
  ...
  const hbs = create({
    defaultLayout: 'main',
    layoutsDir:
      options.handlebarsConfig?.layoutsDir || join(__dirname, 'views'),
