...
export type EndpointInfoRecord = {
    controller: Type<any>;
    prefix: string;
    handler: string;
...
export type HttpEndpointInfo = EndpointInfoRecord & {
    httpPath: string;
    method: string;
};
...
export class EndpointsInfo {
    ...
    static matchHttpRecord(path: string): HttpEndpointInfo[] {
        return EndpointsInfo.httpEndpoints.filter(
            (x) => x.pattern?.test(path),
        );
...
export function completeEndpointsInfo() {
    for (const endpoint of EndpointsInfo.commands) {
        EndpointsInfo._commandsCache[endpoint.command] = endpoint.operation;
    }
    for (const endpoint of EndpointsInfo.events) {
        EndpointsInfo._eventsCache[endpoint.event] = endpoint.operation;
    }
    for (const endpoint of EndpointsInfo.wsEndpoints) {
        EndpointsInfo._wsCache[endpoint.event] = endpoint.operation;
    }
    ...
    for (const x of EndpointsInfo.httpEndpoints) {
        ...
        if (x.method === undefined)
            logger.warn(
                `Undefined method for ${x.controller.constructor.name} ${x.method} ${x.path}`,
            );
        if (x.path === undefined) {
            logger.warn(
                `Undefined path for ${x.controller.constructor.name} ${x.method} ${x.path}`,
            );
            continue;
        }
        ...
        const [hasStar, hasParam] = [
            x.path.includes('*'),
            x.path.includes(':'),
        ];
        let patternString = route;
        if (hasStar) patternString = patternString.replace(/\*+/g, '.*');
        if (hasParam) patternString = patternString.replace(/:[^/]+/g, '[^/]+');
        //then replace :params
