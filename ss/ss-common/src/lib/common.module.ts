...
const redisClients = Object.keys(process.env)
    .filter(
        (name) => name.startsWith('REDIS_') && process.env[name]?.includes(':'),
    )
    .map((provide: string) => {
        return {
            provide,
            useFactory: () =>
                RedisClient.create(provide, parseRedisConfig(provide)),
...
const providers: Provider[] = [
    ...
    Broker,
    {
        ...
        useFactory: (ah: HttpAdapterHost) =>
            ah.httpAdapter?.getInstance().get('io-server'),
...
export class CommonModule {
    static register(
        config: Partial<SSConfig> = SSConfig.default,
    ): DynamicModule {
        ...
        if (_config.broker.startsWith('REDIS_')) {
            ...
            BROKER_CLIENT_PROVIDER = {
                provide: BROKER_CLIENT,
                useFactory: () => new ClientRedis(redisOptions),
            } as Provider;
            imports.push(
