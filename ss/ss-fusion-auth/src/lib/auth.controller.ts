...
@Controller('auth')
export class FusionAuthController {
    ...
    @EndPoint({ http: { method: 'GET', path: 'oauth2-info/:clientId' }, operation: 'getOAuth2Info' })
    public async getOAuth2Info(@Message() msg: IncomingMessage<any>) {
        const { clientId } = msg.query
        const r = await this.auth.getOAuth2Info(clientId)
        const info = {
            ...r.application.oauthConfiguration,
    ...
    @Authorize({ by: 'role', value: 'super-admin' })
    public async admincreateuser(@Message() msg: IncomingMessage<{
        user: Partial<User>,
        password?: string,
        passwordChangeRequired?: boolean
    }>) {

        const { user, password, passwordChangeRequired } = msg.payload;
        const { access, rule, source } = this.authorizationService.authorize(msg, 'create')
        if (access === 'deny') throw new HttpException({ rule, source }, HttpStatus.FORBIDDEN)

        return await this.auth.signUp(user, password, passwordChangeRequired)
    }
    ...
    //         let userRecord: User = await this.auth.findUserByEmail(req.email);
    //         const authUser = await verifyGoogleUser(req.authorization_response.id_token)

    //         if (authUser) {
    //             if (!userRecord) {
    //                 const user = { username: req.email, email: req.email, name: req.name }
    ...
    public async signIn(@Message() msg: IncomingMessage<SignInPayload>) {
        const ip = msg.ctx.ip ? msg.ctx.ip as string : undefined;
        logger.info('signin from: ', ip)
        return await this.auth.signIn(msg.payload, ip)
    }

    @EndPoint({ http: { method: 'POST', path: 'signup' } })
    public async signup(@Message() msg: IncomingMessage<User & { password: string }>) {
        return await this.auth.signUp(msg.payload, msg.payload.password)
        // try {
