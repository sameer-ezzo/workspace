...
export class AuthenticationInterceptor implements NestInterceptor {
    private providers: HttpAuthenticationProvider[];
    
    constructor(@Inject(AuthService) private readonly auth: AuthService) {
        this.providers = [new BearerAuthenticationProvider(this.auth), new CloudflareCookieAuthenticationProvider(this.auth)];
    }
...
export class CookieAuthenticationProvider implements HttpAuthenticationProvider {
    ...
    constructor(
        ...
        public readonly cookieName = "Authorization",
        public readonly jwksPath = "/oauth2/v3/certs",
    ) {}
    async authenticate(req: Request & Record<string, any>): Promise<Principle> {
        ...
        if (header.kid && issuer.startsWith("http")) {
            try {
                ...
                if (error.code != "ERR_JWT_EXPIRED") console.error("CookieAuthenticationProvider.authenticate", error);
            }
        } else {
