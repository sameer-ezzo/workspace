...
import { AppError, ExtractIncomingMessage, toHttpException } from "@ss/common";

function getPrinciple(ctx: ExecutionContext) {
    switch (ctx.getType()) {
        case "http":
            return ctx.switchToHttp().getRequest().principle;
        case "rpc":
            return ctx.switchToRpc().getData().principle;
        case "ws":
            return ctx.switchToWs().getData().principle;
    }
}
...
export class AuthenticatedInterceptor implements NestInterceptor {
    intercept(ctx: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        if (!msg.principle) throw toHttpException(new AppError("Unauthenticated", { code: "UNAUTHENTICATED", status: HttpStatus.UNAUTHORIZED }));
        return next.handle();
    }
}
