...
export class AuthService {
  ...
  async signUp<KeyType extends string | ObjectId = string>(
    user: Partial<User<KeyType>>,
    password?: string,
  ): Promise<{ _id: KeyType } & Partial<User<KeyType>>> {
    ...
    if (errors.length)
      throw new AuthException(AuthExceptions.InvalidSignup, errors);
  ...
  verifyUser(user: Partial<User>): string[] {
    ...
    if (user.phone && !user.phone.startsWith('+'))
      result.push('PHONE_MUST_STARTS_WITH_PLUS');
    return result;
  }
  ...
  async changeUserToRoles(userId: string, roles: string[]) {
    ...
    const rs = roles ?? [];
    await this.data.patch(
      `/user/${user.id}`,
      [{ op: 'replace', path: '/roles', value: rs }],
      user,
    );
  ...
  async addUserToRoles(userId: string, roles: string[]) {
    ...
    const rs = (user.roles || []).concat(
      roles.filter((r) => user.roles?.indexOf(r) < 0),
    );
    if (!roles || roles.length === 0)
      throw new AuthException(AuthExceptions.InvalidRolesData);
    await this.data.patch(
      `/user/${user.id}`,
      [{ op: 'replace', path: '/roles', value: rs }],
      user,
  ...
  async signInUserByUsernameAndPassword(
    username: string,
    password: string,
    device: UserDevice,
  ...
  async signInUserByEmailAndPassword(
    email: string,
    password: string,
    device: UserDevice,
  ): Promise<UserDocument> {
  ...
  async signInUserByPhoneAndPassword(
    phone: string,
    password: string,
    device: UserDevice,
  ): Promise<UserDocument> {
  ...
  async signInUserByIdAndPassword(
    ...
    if (!password && (user.passwordHash || user.email || user.phone)) {
      ...
      throw new AuthException(
        AuthExceptions.INVALID_PASSWORDLESS_SIGNIN_REQUEST,
        '',
      );
    }
    return this.signInUser(user, password, true, device);
  ...
  async signInUserByRefreshToken(refreshToken: string): Promise<UserDocument> {
    ...
    if (token && token.t === 'rfs') {
      ...
      if (user && user.securityCode !== token.sec)
        throw new AuthException(AuthExceptions.InvalidSecurityCode);
  ...
  async signInUserByPrinciple(
    ...
    if (!user) throw new AuthException(AuthExceptions.UserNotFound);
    if (user && user.securityCode !== principle.sec)
      throw new AuthException(AuthExceptions.InvalidSecurityCode);
  ...
  async signInUser(
    ...
    if (isPasswordCorrect) {
      try {
        ...
        logger.error(
          'signInUser could not register success login attempt',
          err,
        );
      }
      return user;
  ...
  async verify(
    ...
    if (verifyToken && value) {
      ...
      if (
        ...
        (verification.code === '616626' || verification.code === verifyToken) &&
        verification.expire > now.getTime()
      ) {
        await user.updateOne({
          $set: {
            [`${name}Verified`]: true,
        ...
        throw new AuthException(
          AuthExceptions.TooManyAttempts,
  ...
  private async _registerSuccessLoginAttempt(
    user: UserDocument,
    device?: string | UserDevice,
  ) {
  ...
  private _validateAttempt(user: UserDocument): boolean {
    ...
    if (
      user.lastAttempt &&
      now - +user.lastAttempt > this.options.maximumAllowedLoginAttemptsExpiry
    ) {
      user.attempts = 0;
